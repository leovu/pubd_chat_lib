...
class Stickers {
  ...
  static List<Widget> _stickers(String name,Function select) {
    ...
    for (int i=1;i<= collection(name);i++) {
      arr.add(InkWell(
          onTap: () async {
            final ByteData byteData = await rootBundle.load('packages/ggi_chat/assets/meme/$name/$i.${type(name)}');
            File file = await writeToFile(byteData,'$name$i.${type(name)}');
            select(file);
          },
          child: Image.asset('packages/ggi_chat/assets/meme/$name/$i.${type(name)}')));
    }
    return arr;
  }
